...
function solution(number) {
  ...
  number.forEach((num, idx, arr) => {
    // const fArr = arr.filter((num_, fIdx) => idx !== fIdx);
    arr.forEach((fNum, fIdx, fArr) => {
      if (idx !== fIdx) {
        const findNum = -(num + fNum);
        const cArr = fArr
          .map((num_, index) => {
            return {
              num: num_,
          ...
          .filter(
            (obj) =>
              obj.index !== idx && obj.index !== fIdx && obj.num === findNum
          );
        if (cArr.length > 0) {
          cArr.forEach((obj) => {
            const item = [idx, fIdx, obj.index].sort((a, b) => a - b).join("");
            answer.add(item);
          });
        }
      }

/**
 * 문제 설명: 개미 군단이 사냥을 나가려고 합니다. 개미군단은 사냥감의 체력에 딱 맞는 병력을 데리고 나가려고 합니다. 장군개미는 5의 공격력을, 병정개미는 3의 공격력을 일개미는 1의 공격력을 가지고 있습니다. 
...
function solution(hp) {
  const powerArr = [5, 3, 1];
  let hpCalc = hp;
  var answer = 0;
  powerArr.forEach((power) => {
    const antCount = Math.floor(hpCalc / power);
    answer = answer + antCount;
    hpCalc = hpCalc % power;
  });
  return answer;
}
